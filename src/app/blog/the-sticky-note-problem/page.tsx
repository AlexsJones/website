    ...
    <div className="min-h-screen bg-[#0d1117] text-slate-200">
      ...
      <article className="max-w-3xl mx-auto px-6 py-16 font-sans">
        ...
        <Prose>
          ...
          <p>
            You don&apos;t need to be a security expert to see what went wrong. You don&apos;t need to be an LLM expert either. You just need to have watched a team of humans work through an incident, however briefly, to know that the <em>shared situational awareness</em> missing here is not a nice-to-have — it&apos;s the entire reason incident response works at all.
          </p>
          ...
          <H2>Everything Is Messaging</H2>

          <p>
            The dominant pattern for multi-agent LLM systems is <strong>orchestration</strong>. A planner decomposes a task, dispatches subtasks to specialised agents, and stitches the results back together. This pattern works fine until it doesn&apos;t — and &quot;until it doesn&apos;t&quot; is closer than most teams want to admit.
          </p>
          ...
          <p>
            <strong>AutoGen</strong> (now merged into Microsoft&apos;s Agent Framework) gives you async event-driven patterns: sequential, concurrent, handoff, group chat, and Magentic-One. More capable than its predecessor, but still fundamentally message-passing. Pattern-based coordination is still explicit messaging, not ambient sharing.
          </p>

          <p>
            <strong>Google A2A</strong> (Agent-to-Agent Protocol) gives you typed task delegation, capability negotiation, and status updates over JSON-RPC 2.0. It&apos;s a message protocol, not a state protocol. It standardises <em>how agents talk</em>, not <em>what they share</em>.
          </p>

          <p>
            <strong>Anthropic MCP</strong> (Model Context Protocol) standardises agent-to-tool communication. It&apos;s foundational for tool access, but orthogonal to coordination. MCP is about agents reaching <em>outwards to tools</em>. Nobody has standardised how agents reach <em>sideways to each other</em>.
          </p>
          ...
          <H2>The Gap Is Named and Measured</H2>
          ...
          <p>
            The <strong>MAST study</strong> (Cemri et al., arXiv:2503.13657, ICLR 2025) compiled 1,600+ annotated failure traces across 7 different frameworks. Three failure clusters emerged: system design, inter-agent misalignment, and task verification. The inter-agent cluster is where the interesting numbers live:
          </p>

          <ul className="list-disc pl-6 space-y-2 my-4">
            <li><strong>13.2%</strong> of failures were reasoning-action mismatches — agents reasoned about one thing and acted on another.</li>
            <li><strong>7.4%</strong> were task derailment — agents lost track of what they were supposed to do.</li>
            <li><strong>6.8%</strong> were wrong-assumption failures — agents assumed facts about the world that weren&apos;t true.</li>
            <li><strong>1.9%</strong> were ignoring other agents entirely.</li>
            <li><strong>0.85%</strong> were information withholding.</li>
          </ul>

          <p>
            The root cause, identified by the authors, is that agents fail at <em>theory of mind</em> — they don&apos;t model what other agents need to know. And the failure mode is unstructured text ambiguity: one agent sends a message, the other interprets it, and something essential is lost in translation.
          </p>
          ...
          <blockquote className="border-l-4 border-emerald-500/60 pl-4 py-1 my-6 text-slate-200 text-[18px] leading-[1.75] italic">
          ...
          <p>
            <strong>Persistent.</strong> The medium itself must outlive any single agent&apos;s session. Without persistence there is no compounding; without compounding there is no collective intelligence. This implies an append-only, event-sourced substrate with full provenance.
          </p>

          <p>
            The thesis reframes coordination from <em>messaging</em> to <em>medium</em>. The interesting object is not the message agents send each other; it is the shared field they live in.
          </p>
          ...
          <H2>The Architecture: Six Layers</H2>

          <p>
            The solution isn&apos;t a single component. It&apos;s a layered architecture — what I call the <strong>synthetic membrane</strong> — six conceptual layers that together provide what biology provides naturally: a shared, permeable boundary.
          </p>

          <pre className="bg-slate-900 border border-slate-800 rounded-lg p-4 overflow-x-auto text-xs font-mono leading-relaxed text-slate-300 my-6">
{`+---------------------------------------------------------------+
|                     L-1: GOVERNANCE                             |
...
+---------------------------------------------------------------+
          ...
          <p>
            <strong>Coordination (L3)</strong> holds the swarm primitives: task broadcast and claim, quorum-sensing thresholds, dynamic group formation and dissolution, and consensus computation. Coordination is multi-mode — shared state, ad-hoc pairwise messaging, and broadcast are all first-class options; agents choose per interaction.
          </p>

          <p>
            <strong>Immune (cross-cutting)</strong> threads through every layer: behavioural anomaly detection, cytokine-style gossip propagation across the coordination layer, memory cells in the registry, and proportional response via gated permeability. Static rules will be routed around; defence must be adaptive.
          </p>
          ...
          <H2>The Incident Commanders Already Knew</H2>
          ...
          <p>
            <strong>Escalation protocols</strong> are the Governance and Immune layers. When an incident exceeds the current commander&apos;s authority, there&apos;s a defined escalation path. Circuit breakers in the membrane serve the same function: when failure cascades exceed a threshold, coordination halts and a human is notified.
          </p>
          ...
          <ul className="list-disc pl-6 space-y-2 my-4">
            <li>A <strong>shared medium</strong> backed by an immutable event log with CRDT convergence</li>
            <li>A <strong>permeability gate</strong> that evaluates whether an agent should read or write a claim</li>
            <li>A <strong>discovery registry</strong> that indexes agents by behavioural evidence</li>
            <li><strong>Coordination primitives</strong> for task broadcast, claim, and quorum sensing</li>
            <li><strong>Governance controls</strong> for circuit breakers and human override</li>
            <li><strong>Immune layer</strong> for anomaly detection and failure attribution</li>
          </ul>
          ...
          <ol className="list-decimal pl-6 space-y-2 my-4">
            <li><strong>Empirical:</strong> MAST measures coordination failures at scale — 1,600+ traces showing that inter-agent misalignment is a primary failure cluster.</li>
            <li><strong>Production:</strong> Framework authors admit their architectures are insufficient — CrewAI&apos;s postmortem, LangGraph debugging complaints, AutoGen&apos;s merge into a new framework.</li>
            <li><strong>Academic:</strong> Blackboard architectures show that shared-medium coordination works, with 13–57% improvement over message-passing approaches.</li>
          </ol>
          ...
          <p>
            <strong>Operational:</strong> Human incident management (ICS/NIMS/SRE) has solved this exact problem for 50+ years, and the solution maps directly to a layered membrane architecture.
          </p>

          <p>
            The synthetic membrane is the hypothesis that brings these threads together. It&apos;s not a rejection of any existing approach — it&apos;s a recognition that messaging and orchestration are necessary but insufficient, and that the medium <em>between</em> agents is the substrate that needs building.
          </p>
        ...
        <div className="mt-16 border-t border-slate-800 pt-8 grid sm:grid-cols-2 gap-4 font-mono text-sm">
          ...
          >
            ...
            <div className="text-slate-200">0001-synthetic-membrane.md · v2.0</div>
          ...
          >
            ...
            <div className="text-slate-500 text-xs mt-1">papers · corpus · entities</div>
          </a>
        </div>
