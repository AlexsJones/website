...
2. **Gated.** Permeability must default to *deny*. The token-economics finding [Bai et al., 2026] shows communication is expensive and that accuracy peaks at intermediate rather than maximal spend; gated-coordination work [Jian et al., 2026] shows that treating communication as a selective, cost-sensitive decision rather than a default reaction improves task outcomes. The membrane must make the agent justify, by cost-benefit, every traversal.
...
The coordination layer holds the swarm primitives: task broadcast and claim, quorum-sensing thresholds, dynamic group formation and dissolution, and consensus computation. Consensus is exposed as a service that surfaces the dissent distribution alongside any headline, per the requirement from §4.1. Coordination is **multi-mode**, informed by [Kashiri et al., 2026] (DM³-Nav), which demonstrates that decentralised coordination without shared state can match centralised baselines on the right tasks. The membrane offers shared state, ad-hoc pairwise messaging, and broadcast as first-class options; agents choose per interaction.

### 4.6 Cross-Cutting: Immune and Observability

Two concerns thread through every layer:

- **Immune defence**, modelled on the vertebrate immune system: behavioural anomaly detection at L0/L1, cytokine-style gossip propagation across L3, memory cells in the registry, proportional response via gated permeability. Co-evolving attacker–defender results [Wen et al., 2026] indicate defence must *adapt*; static rules will be routed around.
...
- **Accuracy peaks at intermediate cost**, then diminishing returns and saturation.
...
- Best-in-class agent-level attribution is 53.5%; step-level is 14.2%. Even OpenAI o1 and DeepSeek R1 fail to reach practical usability.
- Full execution traces improve attribution accuracy by up to **76%** over partial-observation baselines.
- Causal graphs separate root causes from propagated symptoms; counterfactual debugging via targeted intervention is feasible if you have the substrate.
...
Lee and Lee [2026] show, in a controlled human-subjects experiment, that agent *agreement structure* — independent of content — functions as a bias-relevant signal: majority consensus accelerates opinion change and inflates confidence. This makes surfacing dissent a safety requirement, not a nicety. The membrane therefore offers consensus as a service that *always* reports the dissent distribution alongside the headline. Multiple modes (unanimity, supermajority, plurality, interval, defer-to-human) are exposed; the right one is task-dependent. Computing consensus regions with formal guarantees is a research direction we flag but do not claim to have solved.

### 5.7 Decentralisation Is Sometimes Better

Kashiri et al. [2026] (DM³-Nav) demonstrate that, for a decentralised multi-robot semantic-navigation task, agents using only local observations and ad-hoc pairwise messaging — with no central coordinator, global map, or shared global state — match or exceed centralised and shared-map baselines. The membrane therefore is **not** mandated for every interaction. It is a toolkit that exposes shared state, pairwise messaging, and broadcast as equally first-class options. Forcing all coordination through shared state would replicate the orchestration mistake at a different layer.
